...
function retornaPessoasAutorizadas(pessoas) {
    let pessoasAutorizadas = pessoas.filter(function(f) {
        return f.idade > 14 && f.idade < 60 && f.altura >= 1.5
    })
...
function retornaPessoasNaoAutorizadas(pessoas) {
    let pessoasNaoAutorizadas = pessoas.filter(function(f) {
        return f.idade <= 14 || f.idade >= 60 || f.altura < 1.5
    })
...
function retornaContasComSaldoAtualizado(contas) {
    for (j = 0; j < contas.length; j++) {
        contas[j]
        for (i = 0; i < contas[j].compras.length; i++) {
            let soma = 0
            soma = soma + contas[j].compras[i]   
            for(let j in contas) {
                contas[j].saldoTotal = contas[j].saldoTotal - soma
    } 
